...
void Rocks::spawnRock(sf::Vector2f pos, sf::Vector2f window_size) {
	...
	rocks.at(index)->setOrigin(sf::Vector2f(textureSize) / 2.0f);
...
void Rocks::update(float dt, sf::Vector2f window_size, Entity &ship, Pellets &pellets, std::uint16_t &score, int &health) {
	...
	for(int i = rocks.size() - 1; i >= 0; --i) {
		...
		if(ship.getGlobalBounds().findIntersection(rocks.at(i)->getGlobalBounds())) {
			...
			health -= int(rocks.at(i)->getScale().x * textureSize.x);
			deleteRock(i);
			break;
		}
		for(int j = pellets.getPellets().size() - 1; j >= 0; --j) {
			...
			if(pellets.getPellets().at(j)->getGlobalBounds().findIntersection(rocks.at(i)->getGlobalBounds())) {
				...
				if(rocks.at(i)->getScale().x * textureSize.x < 10.0f) {

					deleteRock(i);
					break;
				}
				rocks.at(i)->scale({ 0.5f, 0.5f });
				spawnRock(rocks.at(i)->getPosition(), window_size);

				unsigned int index = rocks.size() - 1;

				rocks.at(index)->setColor(rocks.at(i)->getColor());
				rocks.at(index)->updateEntity(dt, window_size);
				rocks.at(index)->setScale(rocks.at(i)->getScale());
				break;
			}
		}
	}
